...
public class BodyBuilder
{
    ...
    public Body createDynamicCircle(GdxSprite _entity, float _density, float _friction, float _restitution)
    {
        ...
        shape.setRadius((_entity.frameWidth / 2f) / GfxData._PPM);
    ...
    public Body createDynamicBox(GdxSprite _entity, float _density, float _friction, float _restitution)
    {
        ...
        shape.setAsBox
            (
                ((_entity.frameWidth / 2f) / GfxData._PPM),
                ((_entity.frameHeight / 2f) / GfxData._PPM)
            );
     ...
     *
     * @param _entity The {@link GdxSprite} to extract properties from.
     */
    public Body createStaticBody(GdxSprite _entity)
    {
        PolygonShape shape = new PolygonShape();
        shape.setAsBox
            (
     ...
     */
    public Body createStaticBody(GdxSprite _entity, float _density, float _friction, float _restitution)
    {
        PolygonShape shape = new PolygonShape();
        shape.setAsBox
            (
    ...
    private BodyDef createBodyDef(BodyDef.BodyType bodyType, GdxSprite _entity)
    {
        ...
        bodyDef.position.set
            (
                (_entity.sprite.getX() + (_entity.frameWidth / 2f)) / GfxData._PPM,
                (_entity.sprite.getY() + (_entity.frameHeight / 2f)) / GfxData._PPM
            );

        return bodyDef;
    }

    private FixtureDef createFixtureDef(GdxSprite _entity, Shape _shape, float _density, float _friction, float _restitution)
    {
